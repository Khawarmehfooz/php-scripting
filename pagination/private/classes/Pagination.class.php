...
class Pagination {
    ...
    $output = "";
      ...
      for($i=1; $i <= $this->total_pages(); $i++) {
        ...
        if($this->current_page == $i) {
          ...
          $output.= "<a href=\"customers.php?page={$i}\">{$i}</a> ";
        }
      }
      return $output;
  ...
  public function page_links($url=""){
    ...
    if($this->total_pages() > 1){
        ...
        $output.= $this->previous_link('customers__oo.php');
        $output.= $this->number_links('customers__oo.php'); 
        $output.= $this->next_link('customers__oo.php'); 
        $output.= "</p>";
    }
